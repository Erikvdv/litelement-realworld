...
// Actions Interfaces
export interface ActionAddArticleRequested extends Action<'ADD_ARTICLE_REQUESTED'> { article: Article; }
export interface ActionAddArticleCompleted extends Action<'ADD_ARTICLE_COMPLETED'> { }
export interface ActionAddArticleFailed extends Action<'ADD_ARTICLE_FAILED'> { errors: Errors; }
export interface ActionUpdateArticleRequested extends Action<'UPDATE_ARTICLE_REQUESTED'> { article: Article; }
export interface ActionUpdateArticleCompleted extends Action<'UPDATE_ARTICLE_COMPLETED'> { }
export interface ActionUpdateArticleFailed extends Action<'UPDATE_ARTICLE_FAILED'> { errors: Errors; }

export type EditorAction =
    ActionAddArticleRequested | ActionAddArticleCompleted | ActionAddArticleFailed |
    ActionUpdateArticleRequested | ActionUpdateArticleCompleted | ActionUpdateArticleFailed
    ;
...
// Actions
const addArticleFailed: ActionCreator<ActionAddArticleFailed> = (errors: Errors) => {
...
const addArticleRequested: ActionCreator<ActionAddArticleRequested> = (article: Article) => {
    return { type: ADD_ARTICLE_REQUESTED, article };
};

const updateArticleFailed: ActionCreator<ActionUpdateArticleFailed> = (errors: Errors) => {
    return { type: UPDATE_ARTICLE_FAILED, errors };
};

const updateArticleCompleted: ActionCreator<ActionUpdateArticleCompleted> = () => {
    return { type: UPDATE_ARTICLE_COMPLETED };
};

const updateArticleRequested: ActionCreator<ActionUpdateArticleRequested> = (article: Article) => {
...
export const addArticle: ActionCreator<ThunkResult> =
    (article: Article, token: string) => async (dispatch) => {
