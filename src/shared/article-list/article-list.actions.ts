...
// Actions Interfaces
export interface AppActionLoadArticleListRequested
  extends Action<'LOAD_ARTICLE_LIST_REQUESTED'> {
  config: ArticleListQuery;
}
export interface AppActionLoadArticleListCompleted
  extends Action<'LOAD_ARTICLE_LIST_COMPLETED'> {
  articles: Article[];
  articleCount: number;
}

export interface AppActionLoadArticleListFailed
  extends Action<'LOAD_ARTICLE_LIST_FAILED'> {}

export interface AppActionArticleListSetPage
  extends Action<'ARTICLE_LIST_SET_PAGE'> {
  page: number;
}

export interface ArticleListActionSetFavoriteRequested
  extends Action<'SET_FAVORITE_REQUESTED'> {
  slug: string;
}
export interface ArticleListActionSetFavoriteFailed
  extends Action<'SET_FAVORITE_FAILED'> {
  slug: string;
}
export interface ArticleListActionSetFavoriteCompleted
  extends Action<'SET_FAVORITE_COMPLETED'> {
  slug: string;
  article: Article;
}
export interface ArticleListActionDeleteFavoriteRequested
  extends Action<'DELETE_FAVORITE_REQUESTED'> {
  slug: string;
}
export interface ArticleListActionDeleteFavoriteFailed
  extends Action<'DELETE_FAVORITE_FAILED'> {
  slug: string;
}
export interface ArticleListActionDeleteFavoriteCompleted
  extends Action<'DELETE_FAVORITE_COMPLETED'> {
  slug: string;
  article: Article;
}
...
// Actions
const loadArticleListFailed: ActionCreator<
  AppActionLoadArticleListFailed
> = () => {
  return {
...
const loadArticleListCompleted: ActionCreator<
  AppActionLoadArticleListCompleted
...
const loadArticleListRequested: ActionCreator<
  AppActionLoadArticleListRequested
> = (config: ArticleListQuery) => {
  return {
...
const articleListSetPageAction: ActionCreator<AppActionArticleListSetPage> = (
  page: number,
) => {
  return {
...
const setFavoriteRequested: ActionCreator<
  ArticleListActionSetFavoriteRequested
> = (slug: string) => {
  return { type: SET_FAVORITE_REQUESTED, slug: slug };
};

const setFavoriteFailed: ActionCreator<ArticleListActionSetFavoriteFailed> = (
  slug: string,
) => {
  return { type: SET_FAVORITE_FAILED, slug: slug };
};

const setFavoriteCompleted: ActionCreator<
  ArticleListActionSetFavoriteCompleted
> = (slug: string, article: Article) => {
  return { type: SET_FAVORITE_COMPLETED, slug: slug, article: article };
};

const deleteFavoriteRequested: ActionCreator<
  ArticleListActionDeleteFavoriteRequested
> = (slug: string) => {
  return { type: DELETE_FAVORITE_REQUESTED, slug: slug };
};

const deleteFavoriteFailed: ActionCreator<
  ArticleListActionDeleteFavoriteFailed
> = (slug: string) => {
  return { type: DELETE_FAVORITE_FAILED, slug: slug };
};

const deleteFavoriteCompleted: ActionCreator<
  ArticleListActionDeleteFavoriteCompleted
> = (slug: string, article: Article) => {
...
export const fetchArticleList: ActionCreator<ThunkResult> = (
  ...
  fetch(`${API_ROOT}/articles/${slug}`, { headers: headers })
    .then(res => res.json())
    .then((data: FetchArticleListResult) =>
      dispatch(loadArticleListCompleted(data.articles, data.articlesCount)),
    )
    .catch(() => dispatch(loadArticleListFailed()));
};

export const articleListSetPage: ActionCreator<ThunkResult> = (
  page: number,
  query: ArticleListQuery,
) => dispatch => {
...
export const setFavorite: ActionCreator<ThunkResult> = (
  slug: string,
  token: string,
) => dispatch => {
  if (!token) {
...
export const deleteFavorite: ActionCreator<ThunkResult> = (
  slug: string,
  token: string,
) => dispatch => {
  let headers: { [key: string]: string } = {};
