...
import {
  HomeFeedNavigationInput,
  SelectedTab,
} from './home-feed-navigation.component'
...
export class HomeContainer extends connect(store)(LitElement) {
  ...
  stateChanged(state: RootState) {
    ...
    this.feedNavigationInput = {
      ...
      selectedTab:
        this.articleListQuery.type === ArticleListType.feed
          ? SelectedTab.feed
          : this.articleListQuery.filters.tag
          ? SelectedTab.tag
          : SelectedTab.all,
  ...
  setTab(tab: string) {
    store.dispatch(
      navigateArticles(
        {
          type:
            tab === SelectedTab.feed
              ? ArticleListType.feed
              : ArticleListType.all,
          filters: { limit: 10, offset: 0 },
        },
        this.isLoggedIn,
