...
import { followProfile, unFollowProfile } from '../profile/profile.actions';
import {
  setFavorite,
  deleteFavorite,
} from '../shared/article-list/article-list.actions';
...
${this.newComment}</textarea
  ...
  toggleFavorite() {
    this.article!.favorited
      ? store.dispatch(deleteFavorite(this.article!.slug, this.token))
      : store.dispatch(setFavorite(this.article!.slug, this.token));
  }
}
